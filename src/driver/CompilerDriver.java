...
public class CompilerDriver {
    ...
    public static void run(String[] args) {
        ...
        argParser.addArgument("-o", "--output").action(Arguments.storeTrue()).help("write asm to output-file");
        argParser.addArgument("-f", "--ir").action(Arguments.storeTrue()).help("write llvm ir to debug_{$source file name}.ll");
        argParser.addArgument("-d", "--debug").action(Arguments.storeTrue()).help("use debug mode , which will record actions in current path");
        //新的可选指令加在上面
